...
@DisplayName("FrameworkException")
class FrameworkExceptionTest {
    ...
    class Wrap {
        ...
        void returnsNullWhenInputIsNull() {
            assertThat(FrameworkException.wrap(null)).isNull();
        }

        @Test
        @DisplayName("returns same instance when input is already a FrameworkException")
        void returnsSameInstanceWhenAlreadyWrapped() {
            var original = new FrameworkException(new IOException("test"));
            assertThat(FrameworkException.wrap(original)).isSameAs(original);
        }

        @Test
        @DisplayName("wraps RuntimeException cause in FrameworkException")
        void wrapsRuntimeExceptionCause() {
            ...
            var result = FrameworkException.wrap(runtimeException);

            assertThat(result).isInstanceOf(FrameworkException.class);
        ...
        void wrapsNonRuntimeExceptionThrowable() {
            ...
            var result = FrameworkException.wrap(ioException);
            assertThat(result).isInstanceOf(FrameworkException.class);
        ...
        void wrapsPlainException() {
            ...
            var result = FrameworkException.wrap(exception);
            assertThat(result).isInstanceOf(FrameworkException.class);
        ...
        void onlyUnwrapsOneLevel() {
            ...
            var result = FrameworkException.wrap(outerRuntime);
        ...
        void canBeCalledConcurrently() throws InterruptedException {
            ...
            for (int i = 0; i < threads.length; i++) {
                threads[i] = new Thread(() -> reference.set(FrameworkException.wrap(runtimeException)));
                threads[i].start();
            }
            ...
            assertThat(reference.get()).isInstanceOf(FrameworkException.class);
        }
    }
    ...
    class Constructor {
        ...
        void storesCauseAndMessageCorrectly() {
            ...
            var exception = new FrameworkException(cause);
