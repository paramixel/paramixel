...
public class TildePathExpander {
     ...
     */
    public static Path expand(String input) {
        Objects.requireNonNull(input, "input must not be null");
        Arguments.requireNonBlank(input, "input must not be blank");
        ...
        Matcher m = USER_PATTERN.matcher(input);
        ...
        if (m.matches()) {
            ...
            String home;
            try {
                home = IS_MAC ? lookupHomeDirectoryMac(user) : lookupHomeDirectoryLinux(user);
            } catch (IOException e) {
     ...
     */
    private static String lookupHomeDirectoryLinux(String user) throws IOException {
        Process process = new ProcessBuilder("getent", "passwd", user)
     ...
     * @throws IOException if the lookup fails
     */
    private static String lookupHomeDirectoryMac(String user) throws IOException {
        Process process = new ProcessBuilder("dscl", ".", "-read", "/Users/" + user, "NFSHomeDirectory")
                .redirectErrorStream(true)
                .start();
