...
import nonapi.org.paramixel.classgraph.io.github.classgraph.Scanner.ClassfileScanWorkUnit;
...
class Classfile {
    ...
    static class ClassContainment {
         ...
         *            the outer class name.
         */
        public ClassContainment(
                final String innerClassName, final int innerClassModifierBits, final String outerClassName) {
        ...
        if (className != null
            ...
            if (scanSpec.classAcceptReject.isRejected(className)) {
                ...
                if (classResource != null) {
                    ...
                    additionalWorkUnits.add(new ClassfileScanWorkUnit(
                            foundInClasspathElt, classResource, /* isExternalClass = */ true));
                } else {
                    if (log != null) {
     ...
     *            the log
     */
    private void extendScanningUpwardsFromAnnotationParameterValues(
            final Object annotationParamVal, final LogNode log) {
        if (annotationParamVal == null) {
            ...
            scheduleScanningIfExternalClass(
                    ((AnnotationEnumValue) annotationParamVal).getClassName(), "enum class", log);
                        ...
                        for (final AnnotationInfo[] paramAnnInfoArr : methodInfo.parameterAnnotationInfo) {
                            if (paramAnnInfoArr != null && paramAnnInfoArr.length > 0) {
                                for (final AnnotationInfo paramAnnInfo : paramAnnInfoArr) {
                                    scheduleScanningIfExternalClass(
                                            paramAnnInfo.getName(), "method parameter annotation", log);
    ...
    void link(
        ...
        if (className.equals("module-info")) {
            ...
            classInfo = ClassInfo.addScannedClass(
                    className,
                    classModifiers,
                    isExternalClass,
        ...
        if (moduleName != null) {
            ...
            if (moduleInfo == null) {
                moduleNameToModuleInfo.put(
                        moduleName, moduleInfo = new ModuleInfo(classfileResource.getModuleRef(), classpathElement));
            }
            if (isModuleDescriptor) {
     ...
     */
    private String getConstantPoolString(
            final int cpIdx, final boolean replaceSlashWithDot, final boolean stripLSemicolon)
    ...
    private String getConstantPoolString(final int cpIdx, final int subFieldIdx)
        ...
        return intern(reader.readString(
                constantPoolStringOffset + 2L,
                utfLen,
    ...
    private void readConstantPoolEntries(final LogNode log) throws IOException {
        ...
        if (classNameCpIdxs != null) {
            ...
            for (final int cpIdx : classNameCpIdxs) {
                final String refdClassName =
                        getConstantPoolString(cpIdx, /* replaceSlashWithDot = */ true, /* stripLSemicolon = */ false);
                if (refdClassName != null) {
                    if (refdClassName.startsWith("[")) {
                        // Parse array type signature, e.g. "[Ljava.lang.String;" -- uses '.' rather than '/'
                        try {
                            final TypeSignature typeSig =
                                    TypeSignature.parse(refdClassName.replace('.', '/'), /* definingClass = */ null);
                ...
                if (typeSigStr != null) {
                    try {
                        if (typeSigStr.startsWith("L") && typeSigStr.endsWith(";")) {
                            ...
                            final TypeSignature typeSig =
                                    TypeSignature.parse(typeSigStr, /* definingClassName = */ null);
                            // Extract class names from type signature
                            typeSig.findReferencedClassNames(refdClassNames);
                        } else if (typeSigStr.indexOf('(') >= 0 || "<init>".equals(typeSigStr)) {
                            // Parse the type signature
                            final MethodTypeSignature typeSig =
                                    MethodTypeSignature.parse(typeSigStr, /* definingClassName = */ null);
                            // Extract class names from type signature
                            typeSig.findReferencedClassNames(refdClassNames);
                        } else {
                            if (log != null) {
    ...
    private void readBasicClassInfo() throws IOException, ClassfileFormatException, SkipClassException {
        ...
        final boolean isPackage =
                relativePath.regionMatches(relativePath.lastIndexOf('/') + 1, "package-info.class", 0, 18);
    ...
    private void readFields() throws IOException, ClassfileFormatException {
        ...
        for (int i = 0; i < fieldCount; i++) {
            ...
            final boolean getStaticFinalFieldConstValue =
                    scanSpec.enableStaticFinalFieldConstantInitializerValues && fieldIsVisible;
            ...
            if (!fieldIsVisible || (!scanSpec.enableFieldInfo && !getStaticFinalFieldConstValue)) {
                ...
                final char fieldTypeDescriptorFirstChar =
                        (char) getConstantPoolStringFirstByte(fieldTypeDescriptorCpIdx);
                ...
                for (int j = 0; j < attributesCount; j++) {
                    ...
                    if ((getStaticFinalFieldConstValue)
                        ...
                        fieldConstValue =
                                getFieldConstantPoolValue(entryTag[cpIdx], fieldTypeDescriptorFirstChar, cpIdx);
            ...
            if (!methodIsVisible || (!enableMethodInfo && !isAnnotation)) {
                ...
                for (int j = 0; j < attributesCount; j++) {
                    ...
                    if (scanSpec.enableAnnotationInfo
                                        ...
                                        if (targetType == 0x01) {
                                            ...
                                            if (typeParameters != null && typeParameterIndex < typeParameters.size()) {
                                                final TypeParameter typeParameter =
                                                        typeParameters.get(typeParameterIndex);
                                            ...
                                            // method descriptor."
                                            // What the heck, guys.
                                            final List<TypeSignature> parameterTypeSignatures =
                        ...
                        this.annotationParamDefaultValues.add(new AnnotationParameterValue(
                                methodName,
                ...
                if (enableMethodInfo) {
                    ...
                    methodInfoList.add(new MethodInfo(
                            ...
                            methodTypeSignatureStr,
                            methodParameterNames,
                            methodParameterModifiers,
    ...
    private void readClassAttributes() throws IOException, ClassfileFormatException {
        ...
        for (int i = 0; i < attributesCount; i++) {
            ...
            if (scanSpec.enableAnnotationInfo //
                ...
                classContainmentEntries.add(
                        new ClassContainment(className, classModifiers, innermostEnclosingClassName));
    ...
    Classfile(
            ...
            final Set<String> classNamesScheduledForExtendedScanning,
