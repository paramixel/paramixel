...
public class ClassGraphClassLoader extends ClassLoader {
    ...
    ClassGraphClassLoader(final ScanResult scanResult) {
        ...
        final boolean classloadersOverridden =
    ...
    protected Class<?> findClass(final String className)
        ...
        final ClassGraphClassLoader delegateClassGraphClassLoader =
                scanResult.classpathFinder.getDelegateClassGraphClassLoader();
        ...
        if (overrideClassLoaders == null
                && environmentClassLoaderDelegationOrder != null
        ...
        final ClassInfo classInfo =
                scanResult.classNameToClassInfo == null ? null : scanResult.classNameToClassInfo.get(className);
        if (classInfo != null) {
        ...
        final ResourceList classfileResources =
                scanResult.getResourcesWithPath(JarUtils.classNameToClassfilePath(className));
        ...
        if (linkageError != null) {
            if (VersionFinder.OS == OperatingSystem.Windows) {
                ...
                if (msg != null) {
                    ...
                    if (wrongNameIdx > -1) {
                        final String theWrongName = msg.substring(wrongNameIdx + wrongName.length(), msg.length() - 1);
                        if (theWrongName.replace('/', '.').equalsIgnoreCase(className)) {
                            throw new LinkageError(
                                    "You appear to have two classfiles with the same "
                                            ...
                                            + "code is not portable. Class name: " + className,
                                    linkageError);
                        }
                    }
                }
