...
final class GraphvizDotfileGenerator {
    ...
    private static void htmlEncode(
            final CharSequence unsafeStr, final boolean turnNewlineIntoBreak, final StringBuilder buf) {
     ...
     *            the buf
     */
    private static void labelClassNodeHTML(
            final ClassInfo ci,
            final String shape,
        ...
        // Class modifiers
        buf.append("<tr><td><font point-size='12'>")
        ...
        final String darkerColor = String.format(
                "#%s%s%s%s%s%s",
                Integer.toString(r >> 4, 16),
                Integer.toString(r & 0xf, 16),
                Integer.toString(g >> 4, 16),
                Integer.toString(g & 0xf, 16),
                Integer.toString(b >> 4, 16),
        ...
        if (annotationInfo != null && !annotationInfo.isEmpty()) {
            buf.append("<tr><td colspan='3' bgcolor='")
                    .append(darkerColor)
        ...
        if (showMethods && methodInfo != null) {
            ...
            for (int i = methodInfoSorted.size() - 1; i >= 0; --i) {
                ...
                if (name.equals("<clinit>")
                        || name.equals("hashCode") && numParam == 0
                        || name.equals("toString") && numParam == 0
                        || name.equals("equals")
                                && numParam == 1
            ...
            if (!methodInfoSorted.isEmpty()) {
                ...
                for (final MethodInfo mi : methodInfoSorted) {
                    ...
                    if (!mi.getName().equals("<init>")) {
                        ...
                        final TypeSignature resultTypeSig =
                                mi.getTypeSignatureOrTypeDescriptor().getResultType();
                        htmlEncode(
                    ...
                    if (paramInfo.length != 0) {
                        for (int i = 0, wrapPos = 0; i < paramInfo.length; i++) {
                            ...
                            final String paramTypeStr =
                                    useSimpleNames ? paramTypeSig.toStringWithSimpleNames() : paramTypeSig.toString();
     ...
     * @return the string
     */
    static String generateGraphVizDotFile(
            final ClassInfoList classInfoList,
            final float sizeX,
            final float sizeY,
            final boolean showFields,
            final boolean showFieldTypeDependencyEdges,
            final boolean showMethods,
        ...
        for (final ClassInfo node : standardClassNodes) {
            ...
            labelClassNodeHTML(
                    node, "box", STANDARD_CLASS_COLOR, showFields, showMethods, useSimpleNames, scanSpec, buf);
            buf.append(";\n");
        }
        ...
        for (final ClassInfo node : interfaceNodes) {
            buf.append('"').append(node.getName()).append('"');
            labelClassNodeHTML(
                    node, "diamond", INTERFACE_COLOR, showFields, showMethods, useSimpleNames, scanSpec, buf);
            buf.append(";\n");
        }
        ...
        for (final ClassInfo classNode : standardClassNodes) {
            for (final ClassInfo directSuperclassNode :
                    classNode.getSuperclasses().directOnly()) {
                if (directSuperclassNode != null
                        && allVisibleNodes.contains(directSuperclassNode.getName())
                        && !directSuperclassNode.getName().equals("java.lang.Object")) {
                    // class --> superclass
                    buf.append("  \"")
                            .append(classNode.getName())
                            .append("\" -> \"")
                            .append(directSuperclassNode.getName())
                            .append("\" [arrowsize=2.5]\n");
                }
            }

            for (final ClassInfo implementedInterfaceNode :
                    classNode.getInterfaces().directOnly()) {
                ...
                for (final FieldInfo fi : classNode.fieldInfo) {
                    for (final ClassInfo referencedFieldType : fi.findReferencedClassInfo(/* log = */ null)) {
                        if (allVisibleNodes.contains(referencedFieldType.getName())) {
                            ...
                            buf.append("  \"")
                                    .append(referencedFieldType.getName())
                                    .append("\" -> \"")
                                    .append(classNode.getName())
                ...
                for (final MethodInfo mi : classNode.methodInfo) {
                    for (final ClassInfo referencedMethodType : mi.findReferencedClassInfo(/* log = */ null)) {
                        if (allVisibleNodes.contains(referencedMethodType.getName())) {
                            // class --[#] field type (open box)
                            buf.append("  \"")
                                    .append(referencedMethodType.getName())
                                    .append("\" -> \"")
                                    .append(classNode.getName())
        ...
        for (final ClassInfo interfaceNode : interfaceNodes) {
            for (final ClassInfo superinterfaceNode :
                    interfaceNode.getInterfaces().directOnly()) {
                if (allVisibleNodes.contains(superinterfaceNode.getName())) {
                    // interface --<> superinterface
                    buf.append("  \"")
                            .append(interfaceNode.getName())
                            .append("\" -> \"")
                            .append(superinterfaceNode.getName())
                            .append("\" [arrowhead=diamond, arrowsize=2.5]\n");
                }
            }
        }
            ...
            for (final ClassInfo annotationNode : annotationNodes) {
                for (final ClassInfo annotatedClassNode : annotationNode.getClassesWithAnnotationDirectOnly()) {
                    if (allVisibleNodes.contains(annotatedClassNode.getName())) {
                        ...
                        buf.append("  \"")
                                .append(annotatedClassNode.getName())
                                .append("\" -> \"")
                                .append(annotationNode.getName())
                                .append("\" [arrowhead=dot, arrowsize=2.5]\n");
                    }
                }
                for (final ClassInfo classWithMethodAnnotationNode :
                        annotationNode.getClassesWithMethodAnnotationDirectOnly()) {
                    if (allVisibleNodes.contains(classWithMethodAnnotationNode.getName())) {
                        // class with method annotation --o method annotation
                        buf.append("  \"")
                                .append(classWithMethodAnnotationNode.getName())
                                .append("\" -> \"")
                                .append(annotationNode.getName())
                                .append("\" [arrowhead=odot, arrowsize=2.5]\n");
                    }
                }
                for (final ClassInfo classWithMethodAnnotationNode :
                        annotationNode.getClassesWithFieldAnnotationDirectOnly()) {
                    if (allVisibleNodes.contains(classWithMethodAnnotationNode.getName())) {
     ...
     */
    static String generateGraphVizDotFileFromInterClassDependencies(
            final ClassInfoList classInfoList,
            final float sizeX,
            final float sizeY,
        ...
        for (final ClassInfo ci : allVisibleNodes) {
            ...
            buf.append("[shape=")
                    .append(ci.isAnnotation() ? "oval" : ci.isInterface() ? "diamond" : "box")
                    .append(",style=filled,fillcolor=\"#")
                    .append(
                            ci.isAnnotation()
                                    ? ANNOTATION_COLOR
