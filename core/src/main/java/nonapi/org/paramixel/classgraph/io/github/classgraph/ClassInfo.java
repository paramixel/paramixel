...
import nonapi.org.paramixel.classgraph.io.github.classgraph.Classfile.ClassContainment;
import nonapi.org.paramixel.classgraph.io.github.classgraph.Classfile.ClassTypeAnnotationDecorator;
import nonapi.org.paramixel.classgraph.io.github.classgraph.FieldInfoList.FieldInfoFilter;
import nonapi.org.paramixel.classgraph.nonapi.io.github.classgraph.json.Id;
...
public class ClassInfo extends ScanResultObject implements Comparable<ClassInfo>, HasName {
    ...
    private static final ReachableAndDirectlyRelatedClasses NO_REACHABLE_CLASSES = //
            new ReachableAndDirectlyRelatedClasses(
        ...
        if (classInfo == null) {
            if (numArrayDims == 0) {
                ...
                if (baseTypeChar != '\0') {
                    ...
                    try {
                        elementTypeSignature = ClassRefTypeSignature.parse(
                                new Parser(eltTypeSigStr),
     ...
     *            the map from class name to class info
     */
    static void addClassContainment(
            final List<ClassContainment> classContainmentEntries, final Map<String, ClassInfo> classNameToClassInfo) {
        for (final ClassContainment classContainment : classContainmentEntries) {
            final ClassInfo innerClassInfo =
                    ClassInfo.getOrCreateClassInfo(classContainment.innerClassName, classNameToClassInfo);
            innerClassInfo.setModifiers(classContainment.innerClassModifierBits);
            final ClassInfo outerClassInfo =
                    ClassInfo.getOrCreateClassInfo(classContainment.outerClassName, classNameToClassInfo);
    ...
    void addClassAnnotation(
            final AnnotationInfo classAnnotationInfo, final Map<String, ClassInfo> classNameToClassInfo) {
     ...
     *            the map from class name to class info
     */
    private void addFieldOrMethodAnnotationInfo(
        ...
        if (annotationInfoList != null) {
            for (final AnnotationInfo fieldAnnotationInfo : annotationInfoList) {
                final ClassInfo annotationClassInfo =
                        getOrCreateClassInfo(fieldAnnotationInfo.getName(), classNameToClassInfo);
                annotationClassInfo.setModifiers(ANNOTATION_CLASS_MODIFIER);
                // Mark this class as having a field or method with this annotation
                this.addRelatedClass(
                        isField ? RelType.FIELD_ANNOTATIONS : RelType.METHOD_ANNOTATIONS, annotationClassInfo);
    ...
    void addFieldInfo(final FieldInfoList fieldInfoList, final Map<String, ClassInfo> classNameToClassInfo) {
        for (final FieldInfo fi : fieldInfoList) {
            ...
            addFieldOrMethodAnnotationInfo(
                    fi.annotationInfo, /* isField = */ true, fi.getModifiers(), classNameToClassInfo);
        }
    ...
    void addMethodInfo(final MethodInfoList methodInfoList, final Map<String, ClassInfo> classNameToClassInfo) {
        for (final MethodInfo mi : methodInfoList) {
            ...
            addFieldOrMethodAnnotationInfo(
                    mi.annotationInfo, /* isField = */ false, mi.getModifiers(), classNameToClassInfo);
                ...
                for (int i = 0; i < mi.parameterAnnotationInfo.length; i++) {
                    ...
                    if (paramAnnotationInfoArr != null) {
                        for (final AnnotationInfo methodParamAnnotationInfo : paramAnnotationInfoArr) {
                            final ClassInfo annotationClassInfo =
                                    getOrCreateClassInfo(methodParamAnnotationInfo.getName(), classNameToClassInfo);
                            annotationClassInfo.setModifiers(ANNOTATION_CLASS_MODIFIER);
     ...
     */
    static ClassInfo addScannedClass(
        ...
        if (classInfo == null) {
            ...
            classNameToClassInfo.put(
                    className, classInfo = new ClassInfo(className, classModifiers, classfileResource));
        } else {
     ...
     */
    private static Set<ClassInfo> filterClassInfo(
            final Collection<ClassInfo> classes,
            final ScanSpec scanSpec,
            final boolean strictAccept,
    ...
    static class ReachableAndDirectlyRelatedClasses {
         ...
         *            the directly related classes
         */
        private ReachableAndDirectlyRelatedClasses(
                final Set<ClassInfo> reachableClasses, final Set<ClassInfo> directlyRelatedClasses) {
     ...
     */
    private ReachableAndDirectlyRelatedClasses filterClassInfo(
            final RelType relType, final boolean strictAccept, final ClassType... classTypes) {
        ...
        if (relType == RelType.METHOD_ANNOTATIONS
                || relType == RelType.METHOD_PARAMETER_ANNOTATIONS
            ...
            for (final ClassInfo subAnnotation : this.filterClassInfo(
                            RelType.CLASSES_WITH_ANNOTATION, strictAccept, ClassType.ANNOTATION)
                    .reachableClasses) {
        ...
        if (relType == RelType.CLASS_ANNOTATIONS
                || relType == RelType.METHOD_ANNOTATIONS
                || relType == RelType.METHOD_PARAMETER_ANNOTATIONS
                || relType == RelType.FIELD_ANNOTATIONS) {
     ...
     */
    static ClassInfoList getAllImplementedInterfaceClasses(
            final Collection<ClassInfo> classes, final ScanSpec scanSpec) {
        return new ClassInfoList(
                ClassInfo.filterClassInfo(
                        classes, scanSpec, /* strictAccept = */ true, ClassType.IMPLEMENTED_INTERFACE),
                /* sortByName = */ true);
    }
     ...
     */
    static ClassInfoList getAllInterfacesOrAnnotationClasses(
            final Collection<ClassInfo> classes, final ScanSpec scanSpec) {
        return new ClassInfoList(
                ClassInfo.filterClassInfo(
                        classes, scanSpec, /* strictAccept = */ true, ClassType.INTERFACE_OR_ANNOTATION),
                /* sortByName = */ true);
    }
    ...
    private List<ClassInfo> getFieldOverrideOrder(
            final Set<ClassInfo> visited, final List<ClassInfo> overrideOrderOut) {
        if (visited.add(this)) {
    ...
    public ClassInfoList getSuperclasses() {
        return new ClassInfoList(
                this.filterClassInfo(RelType.SUPERCLASSES, /* strictAccept = */ false), /* sortByName = */ false);
    }
    ...
    public ClassInfoList getInnerClasses() {
        return new ClassInfoList(
                this.filterClassInfo(RelType.CONTAINS_INNER_CLASS, /* strictAccept = */ false),
                /* sortByName = */ true);
    }
    ...
    public ClassInfoList getInterfaces() {
        ...
        final ReachableAndDirectlyRelatedClasses implementedInterfaces =
                this.filterClassInfo(RelType.IMPLEMENTED_INTERFACES, /* strictAccept = */ false);
        final Set<ClassInfo> allInterfaces = new LinkedHashSet<>(implementedInterfaces.reachableClasses);
        for (final ClassInfo superclass :
                this.filterClassInfo(RelType.SUPERCLASSES, /* strictAccept = */ false).reachableClasses) {
            final Set<ClassInfo> superclassImplementedInterfaces = superclass.filterClassInfo(
                            RelType.IMPLEMENTED_INTERFACES, /* strictAccept = */ false)
                    .reachableClasses;
    ...
    public ClassInfoList getClassesImplementing() {
        ...
        final ReachableAndDirectlyRelatedClasses implementingClasses =
                this.filterClassInfo(RelType.CLASSES_IMPLEMENTING, /* strictAccept = */ !isExternalClass);
        ...
        for (final ClassInfo implementingClass : implementingClasses.reachableClasses) {
            final Set<ClassInfo> implementingSubclasses = implementingClass.filterClassInfo(
                            RelType.SUBCLASSES, /* strictAccept = */ !implementingClass.isExternalClass)
                    .reachableClasses;
            allImplementingClasses.addAll(implementingSubclasses);
        }
        return new ClassInfoList(
                allImplementingClasses, implementingClasses.directlyRelatedClasses, /* sortByName = */ true);
    }
    ...
    public ClassInfoList getAnnotations() {
        synchronized (this) {
            ...
            final ReachableAndDirectlyRelatedClasses annotationClasses =
                    this.filterClassInfo(RelType.CLASS_ANNOTATIONS, /* strictAccept = */ false);
            ...
            for (final ClassInfo superclass : getSuperclasses()) {
                for (final ClassInfo superclassAnnotation : superclass.filterClassInfo(
                                RelType.CLASS_ANNOTATIONS, /* strictAccept = */ false)
                        .reachableClasses) {
            ...
            if (inheritedSuperclassAnnotations == null) {
                ...
                annotationsRef = new ClassInfoList(
                        inheritedSuperclassAnnotations,
                        annotationClasses.directlyRelatedClasses,
                        /* sortByName = */ true);
            }
            return annotationsRef;
        }
    ...
    private ClassInfoList getFieldOrMethodAnnotations(final RelType relType) {
        ...
        final ReachableAndDirectlyRelatedClasses fieldOrMethodAnnotations =
                this.filterClassInfo(relType, /* strictAccept = */ false, ClassType.ANNOTATION);
        final Set<ClassInfo> fieldOrMethodAnnotationsAndMetaAnnotations =
                new LinkedHashSet<>(fieldOrMethodAnnotations.reachableClasses);
        return new ClassInfoList(
                fieldOrMethodAnnotationsAndMetaAnnotations,
                fieldOrMethodAnnotations.directlyRelatedClasses,
                /* sortByName = */ true);
    }
    ...
    private ClassInfoList getClassesWithFieldOrMethodAnnotation(final RelType relType) {
        ...
        final ReachableAndDirectlyRelatedClasses classesWithDirectlyAnnotatedFieldsOrMethods =
                this.filterClassInfo(relType, /* strictAccept = */ !isExternalClass);
        ...
        if (annotationsWithThisMetaAnnotation.reachableClasses.isEmpty()) {
            ...
            final Set<ClassInfo> allClassesWithAnnotatedOrMetaAnnotatedFieldsOrMethods =
                    new LinkedHashSet<>(classesWithDirectlyAnnotatedFieldsOrMethods.reachableClasses);
            for (final ClassInfo metaAnnotatedAnnotation : annotationsWithThisMetaAnnotation.reachableClasses) {
                allClassesWithAnnotatedOrMetaAnnotatedFieldsOrMethods.addAll(metaAnnotatedAnnotation.filterClassInfo(
                                relType, /* strictAccept = */ !metaAnnotatedAnnotation.isExternalClass)
                        .reachableClasses);
            }
            return new ClassInfoList(
    ...
    public ClassInfoList getClassesWithAnnotation() {
        ...
        final ReachableAndDirectlyRelatedClasses classesWithAnnotation =
        ...
        if (isInherited) {
            ...
            final Set<ClassInfo> classesWithAnnotationAndTheirSubclasses =
                    new LinkedHashSet<>(classesWithAnnotation.reachableClasses);
            ...
            return new ClassInfoList(
                    classesWithAnnotationAndTheirSubclasses,
     ...
     */
    private MethodInfoList getDeclaredMethodInfo(
            final String methodName,
            final boolean getNormalMethods,
            final boolean getConstructorMethods,
            final boolean getStaticInitializerMethods) {
        ...
        if (methodName == null) {
            ...
            for (final MethodInfo mi : methodInfo) {
                ...
                if ((isConstructor && getConstructorMethods)
                        || (isStaticInitializer && getStaticInitializerMethods)
     ...
     */
    private MethodInfoList getMethodInfo(
            final String methodName,
            final boolean getNormalMethods,
            final boolean getConstructorMethods,
            final boolean getStaticInitializerMethods) {
        ...
        for (final ClassInfo ci : getMethodOverrideOrder()) {
            for (final MethodInfo mi : ci.getDeclaredMethodInfo(
                    methodName, getNormalMethods, getConstructorMethods, getStaticInitializerMethods)) {
    ...
    public MethodInfoList getDeclaredMethodInfo() {
        return getDeclaredMethodInfo(
                /* methodName = */ null,
                /* getNormalMethods = */ true,
                /* getConstructorMethods = */ false,
    ...
    public MethodInfoList getDeclaredConstructorInfo() {
        return getDeclaredMethodInfo(
                /* methodName = */ null,
                /* getNormalMethods = */ false,
                /* getConstructorMethods = */ true,
                /* getStaticInitializerMethods = */ false);
    }
    ...
    public ClassInfoList getClassesWithMethodAnnotation() {
        ...
        final Set<ClassInfo> classesWithMethodAnnotation =
                new HashSet<>(getClassesWithFieldOrMethodAnnotation(RelType.CLASSES_WITH_METHOD_ANNOTATION));
        ...
        return new ClassInfoList(
                ...
                new HashSet<>(getClassesWithMethodAnnotationDirectOnly()),
                /* sortByName = */ true);
    }
    ...
    public ClassInfoList getClassesWithMethodParameterAnnotation() {
        // Get all classes that have a method annotated or meta-annotated with this annotation
        final Set<ClassInfo> classesWithMethodParameterAnnotation =
                new HashSet<>(getClassesWithFieldOrMethodAnnotation(RelType.CLASSES_WITH_METHOD_PARAMETER_ANNOTATION));
        // Add subclasses of all classes with a method that is non-privately annotated or meta-annotated with
        // this annotation (non-private methods are inherited)
        for (final ClassInfo classWithNonprivateMethodParameterAnnotationOrMetaAnnotation : //
            ...
            classesWithMethodParameterAnnotation.addAll(
                    classWithNonprivateMethodParameterAnnotationOrMetaAnnotation.getSubclasses());
        }
        return new ClassInfoList(
    ...
    ClassInfoList getClassesWithMethodParameterAnnotationDirectOnly() {
        return new ClassInfoList(
                this.filterClassInfo(
                        RelType.CLASSES_WITH_METHOD_PARAMETER_ANNOTATION, /* strictAccept = */ !isExternalClass),
                /* sortByName = */ true);
    }
    ...
    public List<Object> getEnumConstantObjects() {
        ...
        final ReflectionUtils reflectionUtils = scanResult == null ? new ReflectionUtils() : scanResult.reflectionUtils;
    ...
    public ClassInfoList getClassesWithFieldAnnotation() {
        ...
        final Set<ClassInfo> classesWithMethodAnnotation =
                new HashSet<>(getClassesWithFieldOrMethodAnnotation(RelType.CLASSES_WITH_FIELD_ANNOTATION));
        ...
        synchronized (this) {
            ...
            if (typeSignature == null) {
                try {
                    ...
                    throw new IllegalArgumentException(
                            "Invalid type signature for class " + getName() + " in classpath element "
                                    + getClasspathElementURI() + " : " + typeSignatureStr,
                            e);
                }
            }
        }
    ...
    void handleRepeatableAnnotations(final Set<String> allRepeatableAnnotationNames) {
        if (annotationInfo != null) {
            annotationInfo.handleRepeatableAnnotations(
                    ...
                    this,
                    RelType.CLASS_ANNOTATIONS,
                    RelType.CLASSES_WITH_ANNOTATION,
                    null);
        }
        if (fieldInfo != null) {
            for (final FieldInfo fi : fieldInfo) {
    ...
    protected void toString(final boolean useSimpleNames, final StringBuilder buf) {
        ...
        if (typeSig != null) {
            // Generic classes
            typeSig.toStringInternal(
                    useSimpleNames ? ClassInfo.getSimpleName(name) : name,
                    /* useSimpleNames = */ false,
                    modifiers,
                    isAnnotation(),
                    isInterface(),
                    /* annotationsToExclude = */ annotationInfo,
                    buf);
        } else {
            // Non-generic classes
            TypeUtils.modifiersToString(modifiers, ModifierType.CLASS, /* ignored */ false, buf);
            ...
            final Set<ClassInfo> interfaces = this.filterClassInfo(
                            RelType.IMPLEMENTED_INTERFACES, /* strictAccept = */ false)
