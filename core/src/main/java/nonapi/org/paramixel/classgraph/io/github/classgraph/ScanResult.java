...
import nonapi.org.paramixel.classgraph.nonapi.io.github.classgraph.json.JSONDeserializer;
import nonapi.org.paramixel.classgraph.nonapi.io.github.classgraph.json.JSONSerializer;
import nonapi.org.paramixel.classgraph.nonapi.io.github.classgraph.reflection.ReflectionUtils;
import nonapi.org.paramixel.classgraph.nonapi.io.github.classgraph.scanspec.AcceptReject;
...
public final class ScanResult implements Closeable {
    ...
    private static Set<WeakReference<ScanResult>> nonClosedWeakReferences =
            Collections.newSetFromMap(new ConcurrentHashMap<WeakReference<ScanResult>, Boolean>());
    ...
    ScanResult(
            ...
            final Map<File, Long> fileToLastModified,
        ...
        if (classNameToClassInfo != null) {
            ...
            for (final ClassInfo classInfo : classNameToClassInfo.values()) {
                if (classInfo.isAnnotation() && classInfo.annotationInfo != null) {
                    final AnnotationInfo repeatableMetaAnnotation =
                            classInfo.annotationInfo.get("java.lang.annotation.Repeatable");
        ...
        if (scanSpec.enableInterClassDependencies) {
            for (final ClassInfo ci : new ArrayList<>(classNameToClassInfo.values())) {
                ...
                for (final ClassInfo refdClassInfo : ci.findReferencedClassInfo(log)) {
                    ...
                    if (refdClassInfo != null
                            && !ci.equals(refdClassInfo)
    ...
    public ResourceList getResourcesWithPath(final String resourcePath) {
        ...
        final String path = FileUtils.sanitizeEntryPath(
                resourcePath, /* removeInitialSlash = */ true, /* removeFinalSlash = */ true);
    ...
    public <T> Class<T> loadClass(
            final String className, final Class<T> superclassOrInterfaceType, final boolean returnNullIfClassNotFound)
            throws IllegalArgumentException {
        if (closed.get()) {
            throw new IllegalArgumentException("Cannot use a ScanResult after it has been closed");
        }
    ...
    public static ScanResult fromJSON(final String json) {
        ...
        try (AutoCloseableExecutorService executorService =
                new AutoCloseableExecutorService(ClassGraph.DEFAULT_NUM_WORKER_THREADS)) {
