...
public class LogicalZipFile extends ZipFileSlice {
     ...
     */
    LogicalZipFile(
            final ZipFileSlice zipFileSlice,
            final NestedJarHandler nestedJarHandler,
            final LogNode log,
            final boolean enableMultiReleaseVersions)
    ...
    private void parseManifest(final FastZipEntry manifestZipEntry, final LogNode log)
        ...
        for (int i = 0; i < manifest.length; ) {
            ...
            if (manifest[i] == (byte) '\n' || manifest[i] == (byte) '\r') {
                ...
                final Entry<String, Integer> manifestValueAndEndIdx =
                        getManifestValue(manifest, i + IMPLEMENTATION_TITLE_KEY.length + 1);
                ...
                final Entry<String, Integer> manifestValueAndEndIdx =
                        getManifestValue(manifest, i + SPECIFICATION_TITLE_KEY.length + 1);
                ...
                final Entry<String, Integer> manifestValueAndEndIdx =
                        getManifestValue(manifest, i + CLASS_PATH_KEY.length + 1);
                ...
                final Entry<String, Integer> manifestValueAndEndIdx =
                        getManifestValue(manifest, i + BUNDLE_CLASSPATH_KEY.length + 1);
                ...
                final Entry<String, Integer> manifestValueAndEndIdx =
                        getManifestValue(manifest, i + SPRING_BOOT_CLASSES_KEY.length + 1);
                ...
                final Entry<String, Integer> manifestValueAndEndIdx =
                        getManifestValue(manifest, i + SPRING_BOOT_LIB_KEY.length + 1);
                ...
                final Entry<String, Integer> manifestValueAndEndIdx =
                        getManifestValue(manifest, i + MULTI_RELEASE_KEY.length + 1);
                ...
                final Entry<String, Integer> manifestValueAndEndIdx =
                        getManifestValue(manifest, i + ADD_EXPORTS_KEY.length + 1);
                ...
                final Entry<String, Integer> manifestValueAndEndIdx =
                        getManifestValue(manifest, i + ADD_OPENS_KEY.length + 1);
                addExportsManifestEntryValue = manifestValueAndEndIdx.getKey();
                if (log != null) {
                ...
                final Entry<String, Integer> manifestValueAndEndIdx =
                        getManifestValue(manifest, i + AUTOMATIC_MODULE_NAME_KEY.length + 1);
    ...
    private void readCentralDirectory(final NestedJarHandler nestedJarHandler, final LogNode log)
        ...
        if (eocdPos < 0 && slice.sliceLength > 22 + 32) {
            ...
            try (final ArraySlice arraySlice = new ArraySlice(
        ...
        if (reader.readUnsignedShort(eocdPos + 4) > 0
                || reader.readUnsignedShort(eocdPos + 6) > 0
        ...
        if (zip64cdLocIdx >= 0 && reader.readUnsignedInt(zip64cdLocIdx) == 0x07064b50L) {
            ...
            if (reader.readUnsignedInt(eocdPos64 + 16) > 0
                    || reader.readUnsignedInt(eocdPos64 + 20) > 0
        ...
        try {
            ...
            for (long entOff = 0; entOff + 46 <= cenSize; entOff += entSize) {
                ...
                String entryNameSanitized = FileUtils.sanitizeEntryPath(
                        entryName, /* removeInitialSlash = */ true, /* removeFinalSlash = */ false);
                ...
                // Add zip entry
                final FastZipEntry entry = new FastZipEntry(
                        this,
                        locHeaderPos,
                        entryNameSanitized,
                        isDeflated,
            ...
            if (VersionFinder.JAVA_MAJOR_VERSION < 9) {
                ...
                for (final FastZipEntry versionedZipEntry : entries) {
                    if (!unversionedPathToVersionedPath.containsKey(versionedZipEntry.entryNameUnversioned)) {
                        ...
                        unversionedPathToVersionedPath.put(
                                versionedZipEntry.entryNameUnversioned, versionedZipEntry.entryName);
    ...
    /* (non-Javadoc)
     * @see nonapi.nonapi.org.paramixel.classgraph.io.github.classgraph.fastzipfilereader.ZipFileSlice#toString()
     */
    @Override
