...
public final class ClassTypeSignature extends HierarchicalTypeSignature {
    ...
    ClassTypeSignature(final ClassInfo classInfo, final ClassInfo superclass, final ClassInfoList interfaces) {
        ...
        try {
            superclassSignature = superclass == null
                    ? null
                    : (ClassRefTypeSignature) TypeSignature.parse(
                            "L" + superclass.getName().replace('.', '/') + ";", classInfo.getName());
        ...
        if (interfaces != null) {
            for (final ClassInfo iface : interfaces) {
                try {
                    final ClassRefTypeSignature ifaceSignature = (ClassRefTypeSignature)
                            TypeSignature.parse("L" + iface.getName().replace('.', '/') + ";", classInfo.getName());
    ...
    public int hashCode() {
        return typeParameters.hashCode()
                + (superclassSignature == null ? 1 : superclassSignature.hashCode()) * 7
    ...
    static ClassTypeSignature parse(final String typeDescriptor, final ClassInfo classInfo) throws ParseException {
        ...
        if (parser.hasMore()) {
            ...
            while (parser.hasMore()) {
                ...
                final ClassRefTypeSignature superinterfaceSignature =
                        ClassRefTypeSignature.parse(parser, definingClassNameNull);
        ...
        if (parser.peek() == '^') {
            ...
            while (parser.peek() == '^') {
                parser.expect('^');
                final ClassRefTypeSignature classTypeSignature =
                        ClassRefTypeSignature.parse(parser, classInfo.getName());
                if (classTypeSignature != null) {
                    throwsSignatures.add(classTypeSignature);
                } else {
                    final TypeVariableSignature typeVariableSignature =
                            TypeVariableSignature.parse(parser, classInfo.getName());
        ...
        return new ClassTypeSignature(
                classInfo, typeParameters, superclassSignature, superinterfaceSignatures, throwsSignatures);
    }
}
