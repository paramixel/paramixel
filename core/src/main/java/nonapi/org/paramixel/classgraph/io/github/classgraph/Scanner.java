...
import nonapi.org.paramixel.classgraph.io.github.classgraph.ClassGraph.FailureHandler;
import nonapi.org.paramixel.classgraph.io.github.classgraph.ClassGraph.ScanResultProcessor;
import nonapi.org.paramixel.classgraph.io.github.classgraph.Classfile.ClassfileFormatException;
import nonapi.org.paramixel.classgraph.io.github.classgraph.Classfile.SkipClassException;
import nonapi.org.paramixel.classgraph.nonapi.io.github.classgraph.classpath.ClasspathFinder;
import nonapi.org.paramixel.classgraph.nonapi.io.github.classgraph.classpath.ClasspathOrder.ClasspathEntry;
import nonapi.org.paramixel.classgraph.nonapi.io.github.classgraph.classpath.ModuleFinder;
import nonapi.org.paramixel.classgraph.nonapi.io.github.classgraph.concurrency.AutoCloseableExecutorService;
import nonapi.org.paramixel.classgraph.nonapi.io.github.classgraph.concurrency.InterruptionChecker;
import nonapi.org.paramixel.classgraph.nonapi.io.github.classgraph.concurrency.SingletonMap;
import nonapi.org.paramixel.classgraph.nonapi.io.github.classgraph.concurrency.SingletonMap.NewInstanceFactory;
import nonapi.org.paramixel.classgraph.nonapi.io.github.classgraph.concurrency.WorkQueue;
import nonapi.org.paramixel.classgraph.nonapi.io.github.classgraph.concurrency.WorkQueue.WorkUnitProcessor;
...
class Scanner implements Callable<ScanResult> {
        ...
        try {
            ...
            if (moduleFinder != null) {
                ...
                final ClassLoader[] classLoaderOrderRespectingParentDelegation =
                        classpathFinder.getClassLoaderOrderRespectingParentDelegation();
                        ...
                        (scanSpec.enableSystemJarsAndModules && scanSpec.moduleAcceptReject.acceptAndRejectAreEmpty())
                            ...
                            if (classpathFinderLog != null) {
                                classpathFinderLog.log(
                                        "Skipping non-accepted or rejected system module: " + moduleName);
                            }
                        }
                    }
                }
     ...
     */
    private static void findClasspathOrderRec(
            final ClasspathElement currClasspathElement,
            final Set<ClasspathElement> visitedClasspathElts,
            final List<ClasspathElement> order) {
        if (visitedClasspathElts.add(currClasspathElement)) {
            ...
            final List<ClasspathElement> childClasspathElementsSorted =
                    CollectionUtils.sortCopy(currClasspathElement.childClasspathElements);
     ...
     */
    private <W> void processWorkUnits(
            final Collection<W> workUnits, final LogNode log, final WorkUnitProcessor<W> workUnitProcessor)
            throws InterruptedException, ExecutionException {
        WorkQueue.runWorkQueue(
                workUnits, executorService, interruptionChecker, numParallelTasks, log, workUnitProcessor);
        if (log != null) {
            log.addElapsedTime();
        }
    ...
    private static Object normalizeClasspathEntry(final Object classpathEntryObj) throws IOException {
        ...
        if (!(classpathEntryObjNormalized instanceof Path)) {
            classpathEntryObjNormalized =
                    FastPathResolver.resolve(FileUtils.currDirPath(), classpathEntryObjNormalized.toString());
        }
        ...
        if (classpathEntryObjNormalized instanceof String) {
            ...
            final boolean isURL =
                    JarUtils.URL_SCHEME_PATTERN.matcher(classpathEntStr).matches();
    ...
    private WorkUnitProcessor<ClasspathEntryWorkUnit> newClasspathEntryWorkUnitProcessor(
        ...
        return new WorkUnitProcessor<ClasspathEntryWorkUnit>() {
            ...
            public void processWorkUnit(
                    ...
                    final WorkQueue<ClasspathEntryWorkUnit> workQueue,
                    final LogNode log)
                    throws InterruptedException {
                try {
                    ...
                    if (workUnit.classpathEntryObj instanceof URL || workUnit.classpathEntryObj instanceof URI) {
                        ...
                        if ("JrtFileSystem"
                                .equals(path.getFileSystem().getClass().getSimpleName())) {
                    ...
                    classpathEntryObjToClasspathEntrySingletonMap.get(
                            workUnit.classpathEntryObj,
                            log,
                            ...
                            new NewInstanceFactory<ClasspathElement, IOException>() {
                                ...
                                public ClasspathElement newInstance() throws IOException, InterruptedException {
                                    ...
                                    final LogNode subLog = log == null
                                            ? null
                                            : log.log(
                                                    classpathElement.getURI().toString(),
    ...
    private static class ClassfileScannerWorkUnitProcessor implements WorkUnitProcessor<ClassfileScanWorkUnit> {
         ...
         */
        public ClassfileScannerWorkUnitProcessor(
                final ScanSpec scanSpec,
                final List<ClasspathElement> classpathOrder,
                final Set<String> acceptedClassNamesFound,
         ...
         */
        /* (non-Javadoc)
         * @see nonapi.nonapi.org.paramixel.classgraph.io.github.classgraph.concurrency.WorkQueue.WorkUnitProcessor#processWorkUnit(
         * java.lang.Object, nonapi.nonapi.org.paramixel.classgraph.io.github.classgraph.concurrency.WorkQueue)
         */
        @Override
        public void processWorkUnit(
                final ClassfileScanWorkUnit workUnit,
                final WorkQueue<ClassfileScanWorkUnit> workQueue,
                final LogNode log)
                throws InterruptedException {
            ...
            final LogNode subLog = workUnit.classfileResource.scanLog == null
                    ? null
                    : workUnit.classfileResource.scanLog.log(workUnit.classfileResource.getPath(), "Parsing classfile");

            try {
                // Parse classfile binary format, creating a Classfile object
                final Classfile classfile = new Classfile(
                        workUnit.classpathElement,
                        classpathOrder,
                        acceptedClassNamesFound,
                        classNamesScheduledForExtendedScanning,
                        workUnit.classfileResource.getPath(),
                        workUnit.classfileResource,
                        workUnit.isExternalClass,
                        stringInternMap,
                        workQueue,
                        scanSpec,
                        subLog);
     ...
     *            the log
     */
    private void findNestedClasspathElements(
            final List<SimpleEntry<String, ClasspathElement>> classpathElts, final LogNode log) {
        ...
        CollectionUtils.sortIfNotEmpty(classpathElts, new Comparator<SimpleEntry<String, ClasspathElement>>() {
            @Override
            public int compare(
                    final SimpleEntry<String, ClasspathElement> o1, final SimpleEntry<String, ClasspathElement> o2) {
     ...
     */
    private void preprocessClasspathElementsByType(
            final List<ClasspathElement> finalTraditionalClasspathEltOrder, final LogNode classpathFinderLog) {
     ...
     */
    private ScanResult performScan(
            final List<ClasspathElement> finalClasspathEltOrder,
        ...
        if (scanSpec.enableClassInfo) {
            ...
            for (final ClasspathElement classpathElement : finalClasspathEltOrder) {
                ...
                for (final Resource resource : classpathElement.acceptedClassfileResources) {
                    ...
                    if (!acceptedClassNamesFound.add(className)
            ...
            final ClassfileScannerWorkUnitProcessor classfileWorkUnitProcessor = //
                    new ClassfileScannerWorkUnitProcessor(
                            scanSpec,
                            finalClasspathEltOrder,
                            Collections.unmodifiableSet(acceptedClassNamesFound),
                            scannedClassfiles);
            processWorkUnits(
    ...
    private ScanResult openClasspathElementsThenScan() throws InterruptedException, ExecutionException {
        ...
        final List<ClasspathEntry> rawClasspathOrder =
                classpathFinder.getClasspathOrder().getOrder();
        for (final ClasspathEntry rawClasspathEntry : rawClasspathOrder) {
            rawClasspathEntryWorkUnits.add(new ClasspathEntryWorkUnit(
                    rawClasspathEntry.classpathEntryObj,
                    rawClasspathEntry.classLoader,
                    /* parentClasspathElement = */ null,
        ...
        final Set<ClasspathElement> allClasspathElts =
                Collections.newSetFromMap(new ConcurrentHashMap<ClasspathElement, Boolean>());
        final Set<ClasspathElement> toplevelClasspathElts =
                Collections.newSetFromMap(new ConcurrentHashMap<ClasspathElement, Boolean>());
        processWorkUnits(
        ...
        preprocessClasspathElementsByType(
                classpathEltOrder, topLevelLog == null ? null : topLevelLog.log("Finding nested classpath elements"));

        // Order modules before classpath elements from traditional classpath
        final LogNode classpathOrderLog =
                topLevelLog == null ? null : topLevelLog.log("Final classpath element order:");
        ...
        if (performScan) {
            ...
            return new ScanResult(
                    scanSpec,
                    finalClasspathEltOrderFiltered,
                    finalClasspathEltOrderStrs,
                    classpathFinder,
        ...
        try {
            ...
            if (failureHandler == null) {
                ...
                try {
                    ...
                    final ExecutionException failureHandlerException =
                            new ExecutionException("Exception while calling failure handler", f);
