...
import nonapi.org.paramixel.classgraph.io.github.classgraph.ClassGraph.ClasspathElementFilter;
import nonapi.org.paramixel.classgraph.io.github.classgraph.ClassGraph.ClasspathElementURLFilter;
...
public class ClasspathOrder {
        ...
        if (scanSpec.classpathElementFilters != null) {
            for (final Object filterObj : scanSpec.classpathElementFilters) {
                if ((classpathElementURL != null
                                && filterObj instanceof ClasspathElementURLFilter
                                && !((ClasspathElementURLFilter) filterObj)
                                        .includeClasspathElement(classpathElementURL))
                        || (classpathElementPath != null
                                && filterObj instanceof ClasspathElementFilter
     ...
     * @return true, if added and unique
     */
    private boolean addClasspathEntry(
            final Object pathElement,
        ...
        if (pathElement instanceof URL
                || pathElement instanceof URI
            ...
            if (hasSuffix) {
                try {
                    pathElementWithoutSuffix = pathElement instanceof URL
                            ? new URL(pathElementStrWithoutSuffix)
                            : pathElement instanceof URI
                                    ? new URI(pathElementStrWithoutSuffix)
                                    : pathElement instanceof Path
                                            ? Paths.get(pathElementStrWithoutSuffix)
                    ...
                    try {
                        pathElementWithoutSuffix = pathElement instanceof URL
                                ...
                                : pathElement instanceof URI
                                        ? new URI("file:" + pathElementStrWithoutSuffix)
                                        : pathElementStrWithoutSuffix;
            ...
            final String pathElementStrResolved =
                    FastPathResolver.resolve(FileUtils.currDirPath(), pathElementStrWithoutSuffix);
    ...
    public boolean addClasspathEntry(
            final Object pathElement, final ClassLoader classLoader, final ScanSpec scanSpec, final LogNode log) {
        if (pathElement == null) {
            return false;
        }
        ...
        if (pathElementStr.endsWith("/*") || pathElementStr.endsWith("\\*")) {
            ...
            if (m1.find()) {
                ...
                try {
                    pathElementURL = pathElement instanceof URL
                            ? (URL) pathElement
                            : pathElement instanceof URI
                                    ? ((URI) pathElement).toURL()
                                    : pathElement instanceof Path
                                            ? ((Path) pathElement).toUri().toURL()
                                            : pathElement instanceof File
                                                    ? ((File) pathElement)
                                                            .toURI()
                                                            .toURL()
        ...
        if (pathElementURL != null
            ...
            classpathElementObj = pathElement instanceof File
                    ? pathElementStr
            ...
            if (baseDirFiles != null) {
                for (final File fileInDir : baseDirFiles) {
                    ...
                    if (!name.equals(".") && !name.equals("..")) {
                        ...
                        final String fileInDirPathResolved =
                                FastPathResolver.resolve(FileUtils.currDirPath(), fileInDirPath);
     ...
     */
    public boolean addClasspathEntries(
            final List<Object> overrideClasspath,
            final ClassLoader classLoader,
            final ScanSpec scanSpec,
            final LogNode log) {
     ...
     * @return true (and add the classpath element) if pathElement is not null or empty, otherwise return false.
     */
    public boolean addClasspathPathStr(
            final String pathStr, final ClassLoader classLoader, final ScanSpec scanSpec, final LogNode log) {
    ...
    public boolean addClasspathEntryObject(
            final Object pathObject, final ClassLoader classLoader, final ScanSpec scanSpec, final LogNode log) {
        boolean valid = false;
        if (pathObject != null) {
            if (pathObject instanceof URL
                    || pathObject instanceof URI
                    || pathObject instanceof Path
