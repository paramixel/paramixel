...
import nonapi.org.paramixel.classgraph.nonapi.io.github.classgraph.classpath.SystemJarFinder;
...
public class ClassGraph {
     ...
     */
    private Future<ScanResult> scanAsync(
            final boolean performScan, final ExecutorService executorService, final int numParallelTasks) {
        try {
            return executorService.submit(new Scanner(
                    performScan,
                    scanSpec,
                    executorService,
                    numParallelTasks,
    ...
    public ScanResult scan(final ExecutorService executorService, final int numParallelTasks) {
        try {
            ...
            final ScanResult scanResult =
                    scanAsync(executorService, numParallelTasks).get();
    ...
    ScanResult getClasspathScanResult(final AutoCloseableExecutorService executorService) {
        try {
            final ScanResult scanResult = scanAsync(
                            /* performScan = */ false, executorService, DEFAULT_NUM_WORKER_THREADS)
                    .get();
    ...
    public List<File> getClasspathFiles() {
        try (AutoCloseableExecutorService executorService =
                        new AutoCloseableExecutorService(DEFAULT_NUM_WORKER_THREADS);
