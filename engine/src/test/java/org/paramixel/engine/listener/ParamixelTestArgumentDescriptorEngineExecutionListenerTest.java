...
public class ParamixelTestArgumentDescriptorEngineExecutionListenerTest {
    ...
    public void printsStartAndFinishLines() {
        ...
        final ParamixelTestArgumentDescriptorEngineExecutionListener listener =
                new ParamixelTestArgumentDescriptorEngineExecutionListener(printer);

        listener.executionStarted(arg);
        listener.executionFinished(arg, TestExecutionResult.failed(new RuntimeException("boom")));

        assertThat(out.toString()).contains("A0").contains("FAIL");
    }
}
