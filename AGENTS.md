...
This project uses specification-oriented development. The authoritative system specification lives in `spec/`. Before implementing any feature, you MUST read the relevant spec files. The spec files are loaded automatically via `opencode.json`.
...
- If a feature requires a change to the API model, update `spec/02-data-model.md` after implementing it.
- If a feature adds or changes an API, update `spec/03-api-contracts.md` after implementing it.
