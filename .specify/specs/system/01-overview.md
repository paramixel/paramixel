...
Paramixel is a custom JUnit Platform test engine for parallel, parameterized test execution
with comprehensive lifecycle management. It pairs each argument produced by a
`@Paramixel.ArgumentsCollector` method with every `@Paramixel.Test` method in a class,
executing them concurrently via Java 21 virtual threads. The project ships a Maven plugin
that integrates the engine into the standard Maven `test` phase.

## Module Inventory

| Module (artifactId) | Packaging | Responsibility | Key Dependencies |
|---|---|---|---|
| `paramixel-api` | `jar` | Public annotations and context interfaces consumed by test authors | `junit-platform-commons` |
| `paramixel-engine` | `jar` | JUnit Platform `TestEngine` implementation: discovery, scheduling, execution | `paramixel-api`, `junit-platform-engine`, `junit-platform-launcher`, `slf4j-api`, `ascii-table` |
| `paramixel-maven-plugin` | `maven-plugin` | Maven Mojo that discovers `@Paramixel.TestClass` classes and fires the engine via the JUnit Platform Launcher | `paramixel-api`, `paramixel-engine`, `junit-platform-launcher`, `maven-plugin-api`, `maven-core` |
| `paramixel-tests` | `jar` | Functional/integration tests exercising the engine via the `paramixel-maven-plugin` goal | `paramixel-api` (test), `paramixel-engine` (test) |
| `paramixel-examples` | `jar` | Demonstrative test classes showing API usage, including Testcontainers integration | `paramixel-api` (test), `paramixel-engine` (test), `testcontainers`, `kafka-clients`, `mongodb-driver-sync` |
| `paramixel-benchmarks` | `jar` | JMH performance benchmarks measuring engine throughput and latency | `paramixel-api` (test), `paramixel-engine` (test), `jmh-core` |
...
+-- versions-rules.xml      Dependency version exclusions
