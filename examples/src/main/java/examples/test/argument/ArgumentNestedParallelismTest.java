    ...
    public static Action actionFactory() {
        ...
        for (int i = 0; i < OUTER_PARALLEL_COUNT; i++) {
            ...
            for (int j = 0; j < INNER_PARALLEL_COUNT; j++) {
                int innerIndex = j;
                leafActions.add(Direct.of(
                        "leaf " + outerIndex + "-" + innerIndex,
                        context -> INVOCATIONS.add("leaf " + outerIndex + "-" + innerIndex)));
            }
            innerParallelActions.add(Parallel.of("inner " + outerIndex, List.copyOf(leafActions)));
        }
