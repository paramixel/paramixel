 ...
 * The test step fails with {@link FailException} on the first two attempts and
 * succeeds on the third, verifying that the retry result reports pass with an
 * attempt count of 3.
 */
public class AnnotationRetryTest {
    ...
    /**
     * Builds a single-step action using annotation-based ID lookup.
     *
    ...
    public static Spec<?> factory() {
        ...
        return Instance.of(AnnotationRetryTest.class).child(annotationResolver.byId("retry"));
    }
    ...
    /**
     * Retries a flaky operation that fails the first two times with {@link FailException}.
     */
    @Paramixel.Id("retry")
    public void retry() {
