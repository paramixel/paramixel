...
4. **`ParamixelDiscovery.discoverTests()`** processes each selector, checks for `@Paramixel.TestClass`, validates method signatures via `MethodValidator`, and invokes the selected `@Paramixel.ArgumentsCollector` method to enumerate arguments. **Only ONE `@Paramixel.ArgumentsCollector` method is allowed per class hierarchy** — if multiple exist, the method in the outermost (most specific) subclass is invoked and methods in parent classes are ignored. It builds a four-level descriptor tree (engine → class → argument → method).
