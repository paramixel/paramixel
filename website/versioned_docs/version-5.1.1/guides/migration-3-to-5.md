...
:::note
In 5.x, `Context` is moved to `org.paramixel.api.action`. See [Context API changes](#context-changes-5x) below.
:::
...
:::note
In 5.x, `Context` is moved to `org.paramixel.api.action` and ancestor navigation is removed. See [Context API changes (5.x)](#context-changes-5x) below.
:::
...
### Context API changes (5.x) {#context-changes-5x}
...
- Update `Context` import to `org.paramixel.api.action.Context` in custom actions and context-mode steps.
...
- Replace `getRunDuration()` with `result.descriptor().get().metadata().runDuration()`.
