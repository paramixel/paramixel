...
| `Descriptor` | `org.paramixel.api` | active node in the execution tree |
| `Status` | `org.paramixel.api` | terminal and intermediate statuses |
| `Context` | `org.paramixel.api.action` | accesses configuration, listener, active descriptor, and status setters |
| `Mode` | `org.paramixel.api.action` | `RUN`, `SKIP`, or `ABORT` |
...
3. In `execute(Context)`, set its own status and fire listener callbacks.
...
- [Context and Mode](../api/execution-context)
