...
`Parallel.each()` produces the same tree without the manual loop:

```java
@Paramixel.Factory
public static Action<?> factory() {
    return Parallel.of("NginxTest")
            .parallelism(2)
            .each(Resource.load(NginxTest.class, "docker-images.txt"), image ->
                Instance.of(image, () -> new NginxTestEnvironment(image))
                        .child("setUp", NginxTestEnvironment::setUp)
                        .child("testHttp", NginxTestEnvironment::testHttp)
                        .child("tearDown", NginxTestEnvironment::tearDown))
            .resolve();
