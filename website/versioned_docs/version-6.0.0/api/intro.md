...
`org.paramixel.api.action` contains `Action` (sealed interface with 10 subtypes), `Builder` (sealed interface with 7 subtypes), and built-in action types: `Step`, `Assert`, `Sequence`, `Parallel`, `Scope`, `Static`, `Instance`, `Delay`, `Repeat`, `Timeout`, and the `Each` data-driven utility.
