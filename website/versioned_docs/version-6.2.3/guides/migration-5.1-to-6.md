---
title: Migration 5.1.x to 6.0.0
description: Upgrade plan for moving Paramixel 5.1.x projects to 6.0.0.
---

# Migration 5.1.x to 6.0.0

Use this plan when upgrading a Paramixel 5.1.x project directly to 6.0.0.

## Migration focus

- Replace `Spec` with `Action` and `Builder` concepts.
- Replace `Sequential.of()` with `sequential()`, `Parallel.of()` with `parallel()`.
- Replace `Lifecycle` with `Scope` (`Static` is available but deprecated since 6.2 — prefer `Scope`).
- Replace `AssertTrue`/`AssertFalse` with `assertTrue()`.
...
- Use `step(name, ContextConsumer)` for user logic.
