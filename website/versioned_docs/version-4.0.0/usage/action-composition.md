...
Paramixel test plans are ordinary `Action` trees. In 4.x, ordered composition uses `Container` and concurrent composition uses `Parallel`.
...
public class MyTest {
    ...
    private static Action setup() {
        return Direct.builder("setup").runnable(context -> {}).build();
    }

    private static Action test() {
        return Direct.builder("test").runnable(context -> {}).build();
    }

    private static Action cleanup() {
        return Direct.builder("cleanup").runnable(context -> {}).build();
    }
}
```
...
public class MyTest {
    ...
    private static Action step1() {
        return Direct.builder("step 1").runnable(context -> {}).build();
    }

    private static Action step2() {
        return Direct.builder("step 2").runnable(context -> {}).build();
    }
}
```
...
public class MyTest {
    ...
    private static Action primary() {
        return Direct.builder("primary").runnable(context -> {}).build();
    }

    private static Action after() {
        return Direct.builder("after").runnable(context -> {}).build();
    }
}
```
...
public class MyTest {
    ...
    private static Action before() {
        return Direct.builder("before").runnable(context -> {}).build();
    }
...
public class KafkaTest {
    ...
    private static Action test() {
        return Direct.builder("test")
                .runnable(context -> {
                    KafkaTestEnvironment env = context.getAncestor("../")
                            .getStore()
        ...
        return Direct.builder("after")
                .runnable(context -> {
                    KafkaTestEnvironment env = context.getStore()
                            .remove(ENVIRONMENT, KafkaTestEnvironment.class)
                            .orElseThrow();
                    env.close();
                })
                .build();
