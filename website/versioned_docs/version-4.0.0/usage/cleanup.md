...
- **`reset()`** resets the execution state so the same registered tasks can be run again. Registered callbacks are preserved.
...
cleanup.reset();              // preserved callbacks can now be re-run
CleanupResult result2 = cleanup.run();
```

- **`clear()`** clears all registered callbacks and resets the execution state. Use this to reuse the same instance from scratch.
...
cleanup.run();
cleanup.clear();              // callbacks removed, can add new ones
