...
When the execution context has an instance (from an `Instance` action), the consumer receives the instance object. When no instance is present, the consumer receives the `Context` itself (context mode). See [Context](../api/execution-context) for the full context API.
...
`Sequential.Spec` also provides `each(Iterable, Function)` — a convenience method that maps a collection to child actions, producing an identical tree to a hand-rolled for-loop. See [Data-Driven Testing](./data-driven-testing#each-convenience-method).

`Lifecycle.Spec`, `Instance.Spec`, `Static.Spec`, and `Parallel.Spec` also provide `each()`.
...
Concurrency is controlled by a three-layer model:

1. **Top-level throttle** — when the root action is a `Parallel`, a semaphore (sized to `paramixel.parallelism`) limits how many direct children are in-flight
2. **Per-Parallel admission** — each `Parallel` gates its own children; effective parallelism defaults to `paramixel.parallelism`, and `.parallelism(N)` is capped by it
3. **Depth-based thread pools + leaf permits** — one thread pool per descriptor depth (created lazily when a Parallel schedules children at that depth), each sized to `paramixel.parallelism`; a global leaf semaphore limits concurrent leaf actions

See [Parallel Execution](../guides/parallel-execution#how-paramixelparallelism-works) for the full concurrency model.

The built `Parallel` action provides `parallelism()` to query the configured parallelism limit.

`Parallel.Spec` also provides `each(Iterable, Function)` — a convenience method that maps a collection to child actions, producing an identical tree to a hand-rolled for-loop. `Lifecycle.Spec`, `Instance.Spec`, `Static.Spec`, and `Sequential.Spec` also provide `each()`. See [Data-Driven Testing](./data-driven-testing#each-convenience-method).
...
`Instance` manages the lifecycle of a factory-created object. It creates the instance, makes it available to children via `Context#instance`, and destroys it after execution (auto-closing if it implements `AutoCloseable`). The `Instantiate` and `Destroy` steps are auto-generated.
...
Programmatically, a context-mode `Step` can use `Context#instance(Class)`:
