...
| Package | Contents |
| --- | --- |
| `org.paramixel.api` | Runner, Configuration, Listener, Result, Status, `Paramixel` (nested annotations: `@Factory`, `@Disabled`, `@Tag`, `@Tags`, `@Priority`, `@Id`), `AnnotationResolver`, `Version`, `ThrowingRunnable`, `ThrowingConsumer` |
...
| `org.paramixel.api.action` | Action model — `Action`, `Spec`, `Step`, `Lifecycle`, `Sequential`, `Parallel`, `Instance`, `Static`, `Repeat`, `Timeout`, `Delay`, `AssertTrue`, `AssertFalse`, `Descriptor`, `Metadata` |
...
| `org.paramixel.spi.action` | Custom action SPI — `ExecutionContext`, `Mode` |
