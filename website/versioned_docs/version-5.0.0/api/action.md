...
Discovery binds each action occurrence to a descriptor; execution mutates descriptor metadata through an `ExecutionContext`.
...
The type parameter `T` represents the type consumed by action callbacks. For `Instance<T>` child steps, `T` is the managed instance type. For a `Step` outside an instance, the callback receives the current `ExecutionContext`.
