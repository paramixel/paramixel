---
title: Migration 4.x to 6.0.0
description: Upgrade plan for moving Paramixel 4.x projects to 6.0.0.
---

# Migration 4.x to 6.0.0

Use this plan when upgrading a Paramixel 4.x project directly to 6.0.0.

## Migration focus

- Replace `org.paramixel.core` imports with `org.paramixel.api` imports.
- Replace `@Paramixel.ActionFactory` with `@Paramixel.Factory`.
- Replace `Container` and `Direct` patterns with `Sequential`, `Scope`, `Parallel`, and `Step`.
- Replace `Store`-style shared state with fixture instances exposed through `Instance` and `Context`.
