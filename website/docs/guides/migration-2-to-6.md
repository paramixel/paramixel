---
title: Migration 2.x to 6.0.0
description: Upgrade plan for moving Paramixel 2.x projects to 6.0.0.
---

# Migration 2.x to 6.0.0

Use this plan when upgrading a Paramixel 2.x project directly to 6.0.0.

## Migration focus

- Replace 2.x imports with the 6.0.0 `org.paramixel.api` package set.
- Convert legacy composed actions to `Sequential`, `Parallel`, `Scope`, `Instance` (`Static` is deprecated since 6.2 — use `Scope` instead).
- Move selection rules to regex configuration keys or `Selector` factories.
