---
title: Migration 3.x to 6.0.0
description: Upgrade plan for moving Paramixel 3.x projects to 6.0.0.
---

# Migration 3.x to 6.0.0

Use this plan when upgrading a Paramixel 3.x project directly to 6.0.0.

## Migration focus

- Replace 3.x discovery and action APIs with `@Paramixel.Factory` methods returning `Action` or `Builder`.
- Convert custom executable units to `Step.of(name, ContextConsumer)`.
- Use descriptors from `Result.descriptor()` for tree state instead of older result-tree APIs.
