...
/**
 * Demonstrates parallel-capable lifecycle execution with multiple tests.
 */
@Paramixel.TestClass
public class LifecycleTest3 {
    ...
    /**
     * Executes the first test method with a simulated delay.
     *
     * @param argumentContext the argument context
     * @throws Exception if the simulated delay is interrupted
     */
    @Paramixel.Test
    public void testMethod(final ArgumentContext argumentContext) throws Exception {
        Thread.sleep(2000);

        Object argument = argumentContext.getArgument();
        System.out.println("[TEST_METHOD-1] Executing test with argument: " + argument);
        if (argument != null) {
            System.out.println(
                    "[TEST_METHOD-1] Argument class: " + argument.getClass().getName());
        }
    }

    /**
     * Executes the second test method with a simulated delay.
     *
     * @param argumentContext the argument context
     * @throws Exception if the simulated delay is interrupted
     */
    @Paramixel.Test
    public void testMethod2(final ArgumentContext argumentContext) throws Exception {
        Thread.sleep(2000);

        Object argument = argumentContext.getArgument();
        System.out.println("[TEST_METHOD-2] Executing test with argument: " + argument);
        if (argument != null) {
            System.out.println(
                    "[TEST_METHOD-2] Argument class: " + argument.getClass().getName());
        }
    }
